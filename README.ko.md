...
| --- | --- | --- | --- |
| 절기는 '날짜'가 아니라 '시각' (입춘 2024 = 2/4 17:26 KST) | 일 단위 적용 → 그날 낮 출생자의 년주가 틀림 | 천문 계산한 절기 시각과 분 단위 비교로 년주·월주 교정 | `npx k-saju 2024-02-04 04:00` → 년주 癸卯 유지 |
...
| 해외 출생 — 절기는 절대 시각 (입춘 2024 = 뉴욕 2/4 03:26 EST) | 현지 '날짜'와 KST 절기 '날짜' 비교 | 절기 시각을 현지시로 환산해 시점끼리 비교 | `npx k-saju 2024-02-04 00:30 --place new-york` → 현지 2/4 인데도 년주 癸卯 유지 |
...
ESM 전용, Node ≥ 20, 타입 내장, 런타임 의존성 2개(모두 MIT):
[@fullstackfamily/manseryeok](https://www.npmjs.com/package/@fullstackfamily/manseryeok)(KASI 기반 1900~2050 — 일주·음양력 변환)
· [astronomy-engine](https://www.npmjs.com/package/astronomy-engine)(절기 시각 천문 계산).

## 정직한 한계

- 절기 시각은 표가 아니라 **천문 계산**(astronomy-engine, 1900~2050 — 범위 밖은 일 단위 폴백). KASI 유래 2026년 값과 1분 이내 일치(KASI 는 분 단위 발표라 1분 차이는 반올림). 계산으로 바꾼 이유: 만세력 데이터셋의 절기 표는 2020~2030 열한 해 전부 2026년 값을 돌려준다 — 0.1.0 은 이 표를 믿어 2026년 외 절기 당일 출생자가 틀렸다(0.1.1 에서 수정).
- **년주·월주는 절기 시각에서 직접 계산하고, 일주만 만세력 라이브러리를 쓴다.** 이유: 데이터셋의 일(日) 단위 월주 표가 절기 **이튿날**에 넘어가는 절기가 많아(경칩 2024 = 3/5 11:22 KST, 표는 3/6 전환) 0.1.1 의 '절기 당일·시각 이전' 보정만으로는 `2024-03-05 12:00` 이 옛 달(丙寅, 정답 丁卯), `2025-02-03 23:30` 이 옛 해(甲辰, 정답 乙巳)로 나왔다. 0.1.2 에서 수정 — 경계 밖 1905~2049 전 일자에서 둘은 일치하고(스윕 자체가 테스트), 데이터셋 년간이 깨진 12개 해의 12/31 만 이제 우회된다.
- 대운수는 절입까지 일수를 천문 절기 시각으로 잰다(순행 올림·역행 내림·시간 미상은 정오). 대운 시작은 학파별 ±1년 차이가 있으니 근거인 `daysToTerm`을 그대로 반환.
- 균시차는 표준 근사식(±0.5분).
- 채택 학파: 야자시=시간만 익일 · 해외출생=현지시 기준 · 대운 방향=년간 음양×성별. 다른 학파면 코드가 짧으니 fork해서 규칙을 바꾸세요.

## 라이선스

MIT. 만세력 데이터셋은 @fullstackfamily/manseryeok(MIT), 절기 천문 계산은 astronomy-engine(MIT).
