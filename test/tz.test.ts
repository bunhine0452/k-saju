...
// times) are converted to local time to re-judge the year/month pillars; day/hour
// pillars follow the local date & time (local-time school).
// 입춘 2024 = Feb 4, 05:02 KST → New York (EST, −300) Feb 3, 15:02.
...
test('tz — New York 입춘 boundary: before local Feb 3 15:02 stays 癸卯, after flips 甲辰', () => {
  // Local Feb 3, 14:00 (= KST Feb 4, 04:00 — before the term) → 癸卯.
  // Here the library's day-granular judgment happens to agree.
  ...
  // Local Feb 3, 16:00 (= KST Feb 4, 06:00 — after the term) → 甲辰. The local date (Feb 3)
  // precedes the term date (Feb 4) so the library keeps 癸卯 — the absolute-instant
  // comparison must correct it to 甲辰 (the branch this engine adds).
  const after = deriveSaju({ date: '2024-02-03', time: '16:00', calendar: 'solar', ...NY });
  ...
  assert.equal(after.solarTermAdjusted, true, 'solar-term adjustment flagged');

  // Day pillar follows the local date — equal to KST Feb 3's day pillar, not Feb 4's.
  const kstSameDate = deriveSaju({ date: '2024-02-03', time: '12:00', calendar: 'solar' });
  assert.equal(after.day.hanja, kstSameDate.day.hanja, 'day pillar follows the local date');
});

test('tz — the reverse direction (library flipped early) is corrected too', () => {
  // Local Feb 4, 00:30 (= KST Feb 4, 14:30 — after the term) → 甲辰; local date = term date,
  // so the library also says 甲辰. Agreement case.
  const a = deriveSaju({ date: '2024-02-04', time: '00:30', calendar: 'solar', ...NY });
  assert.equal(a.year.hanja, '甲辰');
  // Sydney (February AEDT, +660): the term converts to local Feb 4, 07:02. Local Feb 4,
  // 06:00 (= KST Feb 4, 04:00 — before the term): the library flips by date (Feb 4) to 甲辰
